...
const SingleFood = () => {
    ...
    const fetchFood = async () => {
        let found_transaction: FoodModel[] | null = await FoodBloc(accessToken ?? "", uuid).then(item => {
            setIsLoading(false);
            return item;
        });
        if (found_transaction && found_transaction.length > 0) {
            setFood(found_transaction[0]);
            setPrice(PriceFormatter(found_transaction[0].price ?? 0));
            setFoodData({
                name: found_transaction[0].name,
                preparation_time: found_transaction[0].preparationTime,
                details: found_transaction[0].details,
            })
        }
    }
                            ...
                            <form className='form' ref={formRef} onSubmit={updateForm}>
...

                                {
                                    categories.map((item) => {
                                        ...
                                        >{item.title}{item.pk}{food?.category}</option>
                                    })
                                }
                            </FormGroup>
                                <div className='row'>
